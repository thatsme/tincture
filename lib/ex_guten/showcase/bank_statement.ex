defmodule ExGuten.Showcase.BankStatement do
  @moduledoc false
  ...
  def build_document(variant) when variant in [:retail_spending, :joint_fee_interest] do
    ...
    try do
      pdf =
        ...
        |> ExGuten.image_png(48, 736, 18, 18, logo_path)
        |> ExGuten.set_font("Helvetica-Bold", 16)
        |> ExGuten.text_at(72, 734, config.hero_title)
        |> ExGuten.set_font("Helvetica", 10)
        |> ExGuten.text_at(48, 716, config.account_line)
        |> ExGuten.text_at(48, 702, config.period_line)
        |> ExGuten.text_at_rotated(448, 716, 25, config.stamp_text)
      ...
      pdf =
        ...
        |> ExGuten.text_at(48, 242, "Support and Disclosures")
        |> ExGuten.set_font("Helvetica", 9)
        |> ExGuten.text_at(48, 228, "Customer support: (800) 555-0199")
        |> ExGuten.text_at(48, 216, "Fraud hotline: (800) 555-0411")
        |> ExGuten.text_at(48, 204, "This statement includes simulated transactions for testing.")
  ...
  defp render_transactions(pdf, rows, config) do
    Enum.reduce(1..3, {pdf, []}, fn page, {acc_pdf, acc_results} ->
      ...
      page_pdf =
        acc_pdf
        |> ExGuten.set_page(page)
        |> maybe_draw_page_chrome(page, 3, config)
        |> ExGuten.set_font("Helvetica-Bold", 10)
        |> ExGuten.text_at(48, table_top + 16, "Transactions (Page #{page})")
  ...
  defp add_bookmarks(pdf, bookmarks) do
    Enum.reduce(bookmarks, pdf, fn {title, page}, acc ->
      ExGuten.add_bookmark(acc, title, page)
    end)
  end
  ...
  defp draw_page_chrome(pdf, page, total, config) do
    pdf
    |> ExGuten.set_font("Helvetica-Bold", 11)
    |> ExGuten.text_at(48, 758, interpolate_page(config.header_text, page, total))
    |> ExGuten.set_font("Helvetica", 9)
    |> ExGuten.text_at(48, 30, interpolate_page(config.footer_text, page, total))
  end
  ...
  defp write_test_logo_png! do
    path =
      Path.join(
        System.tmp_dir!(),
        "ex_guten_statement_logo_#{System.unique_integer([:positive])}.png"
      )
