defmodule ExGuten.Layout.Template do
  @moduledoc false

  alias ExGuten.Layout.Box
  alias ExGuten.Layout.Box.FlowResult
  alias ExGuten.PDF
  ...
  def render(%PDF{} = pdf, %__MODULE__{} = template, %RichText{} = rich_text, opts \\ [])
    ...
    with_header =
      case template.header do
        ...
        %Slot{} = slot ->
          pdf
          |> ExGuten.set_font(slot.font, slot.size)
          |> ExGuten.text_at(
    ...
    {final_pdf, final_rich, page_results} =
      Enum.reduce_while(0..(max_pages - 1), {pdf, rich_text, []}, fn idx,
                                                                     {acc_pdf, current_rich,
                                                                      results} ->
        current_pdf = if idx == 0, do: acc_pdf, else: ExGuten.add_page(acc_pdf)
