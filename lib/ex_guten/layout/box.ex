defmodule ExGuten.Layout.Box do
  ...
  defp render_token(%Word{} = word, pdf, cursor_x, line_y, rotate) do
    next_pdf =
      pdf
      |> ExGuten.set_font(word.font, word.size)
      |> then(fn doc ->
        case rotate do
          degrees when is_number(degrees) ->
            ExGuten.text_at_rotated(doc, cursor_x, line_y, degrees, word.text)

          _ ->
            ExGuten.text_at(doc, cursor_x, line_y, word.text)
        end
      end)
